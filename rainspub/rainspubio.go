...
//writeZonefile writes sections in zonefileformat to path
func writeZonefile(path string, sections []rainslib.MessageSectionWithSigForward) error {
	var encodings []string
	for _, section := range sections {
		encodings = append(encodings, parser.Encode(section))
	}
	return ioutil.WriteFile(path, []byte(strings.Join(encodings, "\n")), 0600)
}

...
type Set struct {
	...
	data      map[string]sections.Hasher
}

//New creates a new data container
func New() *Set {
	return &Set{isDeleted: false, data: make(map[string]sections.Hasher)}
}
...
func (set *Set) Add(item sections.Hasher) bool {
	set.mux.Lock()
...
func (set *Set) Delete(item sections.Hasher) bool {
	set.mux.Lock()
	defer set.mux.Unlock()
...
func (set *Set) GetAll() []sections.Hasher {
	set.mux.RLock()
	defer set.mux.RUnlock()
	if set.isDeleted {
		return []sections.Hasher{}
	}
	var data []sections.Hasher
...
func (set *Set) GetAllAndDelete() []sections.Hasher {
	set.mux.Lock()
	defer set.mux.Unlock()
	if set.isDeleted {
		return []sections.Hasher{}

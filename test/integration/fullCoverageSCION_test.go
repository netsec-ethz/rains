...
func TestFullCoverageSCION(t *testing.T) {
	...
	stopAuthServers(rootServer, chServer, ethzChServer)
...
func TestFullCoverageCLIToolsSCION(t *testing.T) {
	...
	root_addr, err := snet.AddrFromString(strings.Replace(root_config.ServerAddress.Addr.String(), " (UDP)", "", 1))
	if err != nil {
		t.Fatalf("Was not able to load ServerAddress from namingServerRoot config: %v, %v", err, root_config.ServerAddress.Addr)
	}
	root_host_addr := fmt.Sprintf("%s,[%v]", root_addr.IA, root_addr.Host.L3)
	root_port := root_addr.Host.L4.Port()
	cmd := exec.Command("/bin/bash", "-c",
		fmt.Sprintf("%s/rainsd "+
			"./testdata/conf/SCIONnamingServerRoot.conf --id nameServerRoot", tool_dir))
	...
	cmd = exec.Command("/bin/bash", "-c", fmt.Sprintf("%s/zonepub "+
		"./testdata/conf/SCIONpublisherRoot.conf", tool_dir))
	if err := cmd.Run(); err != nil {
	...
	for _, zone := range []string{"ch", "ethz.ch"} {
		cmd = exec.Command("/bin/bash", "-c", fmt.Sprintf("%s/rainsd "+
			...
			"--rootServerAddress %s:%d --id nameServer%[2]s", tool_dir, zone,
		...
		cmd = exec.Command("/bin/bash", "-c", fmt.Sprintf("%s/zonepub "+
			"./testdata/conf/SCIONpublisher%s.conf", tool_dir, zone))
		if err := cmd.Run(); err != nil {
			t.Fatalf("Error during zonepub %v: %v", "zonepub", err)
		}
		time.Sleep(1000 * time.Millisecond)
	...
	resolver_addr, err := snet.AddrFromString(strings.Replace(resolver_config.ServerAddress.Addr.String(), " (UDP)", "", 1))
	if err != nil {
	...
	cmd = exec.Command("/bin/bash", "-c", fmt.Sprintf("%s/rainsd "+
		...
		"--rootServerAddress %s:%d --id resolver", tool_dir, root_host_addr, root_port))
	if err := cmd.Start(); err != nil {
		t.Fatalf("Error during rainsd %v: %v", "rainsd", err)
	}
	commands = append(commands, cmd)
	...
	for i, rquery := range queries {
		...
		log.Info("Expected", expected_ans, "Got", rdig_answer)
...
func startAuthServer(t *testing.T, name string, rootServers []net.Addr) *rainsd.Server {
	...
	if len(resolver.RootNameServers) > 0 && resolver.RootNameServers[0] == nil {
		log.Error(fmt.Sprintf("Started name server %s with nil root name server: %v", name, resolver.RootNameServers[0]))
	}
...
func stopAuthServers(rootServer, chServer, ethzChServer *rainsd.Server) {

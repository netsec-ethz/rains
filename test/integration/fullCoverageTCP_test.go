...
func TestFullCoverageCLITools(t *testing.T) {
	...
	if err != nil {
		...
		log.Info(fmt.Sprintf("Created tmp dir: %v", tool_dir))
	}
	...
	for i, rquery := range queries {
		...
		qtype, err := ParseTypes(rquery.Types[0].String())
		if err != nil {
...
//ParseTypes returns the CLI type string corresponding to the object type specified in qType
func ParseTypes(qType string) (string, error) {
	switch qType {
	case "OTName":
		return "name", nil
	case "OTIP6Addr":
	...
	return "", fmt.Errorf("%s is not a valid type", qType)
}
